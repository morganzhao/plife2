...
class ProductLogic extends \Think\Model{
    ...
    public function getProductListByCategoryId($cond=array(),$userid){
        ...
        $result = $this->Product->join('left join pn2_category on pn2_category.id = pn2_product.categoryid')->field('pn2_category.name as categoryname,pn2_product.*')->where($mycond)->select();
    ...
    public function getProductList($userid){
        $result[0] = $this->getProductListByCategoryId(array('category'=>7),$userid);
        $result[1] = $this->getProductListByCategoryId(array('category'=>8),$userid);
        return $result;
    }
